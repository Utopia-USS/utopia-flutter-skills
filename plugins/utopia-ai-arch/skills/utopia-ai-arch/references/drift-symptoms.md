...
**A catalogue of failure modes that have actually happened** in production-repo-A,
production-repo-B, and production-repo-C. Each entry has a verbatim quote with
...
> `production-repo-A/.claude/docs/claude-architecture.md:126`
...
keeps only what's truly master-specific (repo-A kept `ffi-conventions.md`,
...
> `production-repo-A/.claude/docs/claude-architecture.md:244`
...
> turn." — `production-repo-A/.claude/docs/claude-architecture.md:222-223`
...
> confuse the agent." —
> `production-repo-B/.claude/docs/claude-architecture.md:138-141`
...
> `production-repo-C/.claude/docs/claude-architecture.md:122-124`
...
**only to legitimise a `.claude/refs/<contract>.md`** — repoB's
`repoB-api` exists primarily to give `proto-contract.md` a logical
...
> today." — `production-repo-B/.claude/docs/claude-architecture.md:148-152`

> "No recent incident has cost enough to warrant a dedicated read-only
> pass. The standard reviewer + precommit auditor cover these surfaces
> today." — `production-repo-C/.claude/docs/claude-architecture.md:75`
...
didn't catch"). repo-A's `bp-security-auditor` is the precedent for when
this **is** justified — KEX FFI, post-quantum KEM, Supabase RLS, push-payload
...
**Evidence (real precedent):**
> "**Do NOT run `mcp__repoB-dart__dart_fix`** as a mandatory step. It
...
> `production-repo-B/.claude/agents/repoB-maintainer.md:38-46`
...
> user's WIP." — `production-repo-B/.claude/agents/repoB-reviewer.md:42-44`
...
> `production-repo-B/.claude/skills/repoB/references/worktree-gotchas.md:9-20`
...
> `production-repo-B/.claude/scripts/dart_mcp_setup.sh:4-10`
...
See repoB's `dart_mcp_setup.sh` for the template; replicate per-repo
...
> forgotten." — `production-repo-A/.claude/agents/bp-maintainer.md:170-179`

> "If the comment wouldn't make sense to a reader who has never seen
> this conversation, PR, or review thread — delete it." —
> `production-repo-A/.claude/agents/bp-maintainer.md:188-189`

The same rule is repeated verbatim in
`production-repo-B/.claude/agents/repoB-maintainer.md:144-164` and
mirrored in `repoB-reviewer.md:45-48` as WARN-grade enforcement.
...
**Evidence (real precedent):**
> "When `/repoB-implement` invokes the reviewer, it withholds this
...
> `production-repo-B/.claude/agents/repoB-maintainer.md:222-227`
...
> `production-repo-B/.claude/docs/claude-architecture.md:122-126`
...
> `production-repo-C/.claude/docs/claude-architecture.md:114-117`
...
> `production-repo-A/.claude/docs/claude-architecture.md:256-258`
...
> `production-repo-B/.claude/docs/claude-architecture.md:108-113`
...
> `production-repo-C/.claude/docs/claude-architecture.md:147-152`

**Why it happens:**
Cargo-culting from another repo. repoB has `mcp__repoB-dart__*`
everywhere; repoC copies the agent prompts and inherits the MCP
...
> `production-repo-A/.claude/docs/claude-architecture.md:235-237`
...
> `production-repo-A/.claude/docs/claude-architecture.md:249-251`
...
> wastes a hook firing. **Reversal criterion.** `repoB-api` (or
> distributors) accumulates 2+ references — wire path nudges then." —
> `production-repo-B/.claude/docs/claude-architecture.md:170-178`

> "`repoC-functions` is primitive — no reference worth nudging the
> agent to read. Adding a nudge that points at 'no content yet'
> wastes a hook firing. **Reversal criterion.** `repoC-functions`
> accumulates 2+ references — wire path nudges then." —
> `production-repo-C/.claude/docs/claude-architecture.md:141-145`
...
repo-A **did** add `bp-security-auditor` (symptom F's reversal met).
repo-A **does** maintain a release playbook as a *skill* (symptom Q's
...
- Source documents (the precedent corpus):
  ...
  - `production-repo-A/.claude/agents/bp-maintainer.md` (comment-style)
  - `production-repo-B/.claude/agents/repoB-maintainer.md` (dart_fix, reviewer-leak)
  - `production-repo-B/.claude/agents/repoB-reviewer.md` (dart_fix BLOCKER)
  - `production-repo-B/.claude/scripts/dart_mcp_setup.sh` (memory leak)
