...
A third script (`<prefix>_session_setup.sh` / `dart_mcp_setup.sh`) is **optional** — `SessionStart` only, only when a measurable local-resource leak has been observed. Jolly's precedent below.
...
> "All other rules are `warn` (exit 1). Mode switch via env var (`BP_QUALITY_MODE=block`) for CI-grade use later." — `qbt-black-phone/.claude/docs/claude-architecture.md:122`
...
| **Basename match against `<repo-folder-name>`** | **The hook fires across every project Claude has open at once.** Without this guard, editing a Dart file in `~/IdeaProjects/some-other-repo/lib/main.dart` would trigger `bp_quality_check.sh` (it's the same script under `~/IdeaProjects/qbt-black-phone/.claude/scripts/`, but Claude points the hook at `${CLAUDE_PROJECT_DIR}`). The basename guard is the cheapest reliable scope check. |

> "Only fire inside the jolly-phonics-apps repo: `[[ \"$(basename \"$repo_root\")\" == \"jolly-phonics-apps\" ]] || exit 0`" — `jolly-phonics-apps/.claude/scripts/jolly_quality_check.sh:76-77`
...
|------|--------------------|---------------------|
| qbt-black-phone | `*.g.dart`, `*.freezed.dart`, `*.gr.dart`, `*.config.dart`, `*.pb.dart`, `*.pbenum.dart`, `*.pbjson.dart`, `*.pbserver.dart` | `dart run build_runner build --delete-conflicting-outputs --workspace` |
| jolly-phonics-apps | `*.pb.dart`, `*.pbenum.dart`, `*.pbjson.dart`, `*.pbserver.dart`, `*.freezed.dart`, `*.g.dart`, `*.gr.dart`, `*.config.dart` | `melos run build_runner:build` |
| madrosc-tlumu | `*.freezed.dart`, `*.g.dart`, `*.config.dart` | `fvm dart run build_runner build --delete-conflicting-outputs` (run inside affected package) |
...
> "uses relative Dart import — repo convention requires 'package:...' imports (always_use_package_imports)" — `qbt-black-phone/.claude/scripts/bp_quality_check.sh:98`
...
- **qbt**: 5+ nudges covering UI paths, state files, security-sensitive crypto/FFI paths, message_service, supabase paths
- **jolly**: 5 nudges covering activity proto+UI, Crazy UI / classroom non-lesson, services, models, proto edits
- **tlumu**: 6 nudges covering game-flow RTDB paths, design-system, decks/trivia, services, models, IAP
...
> "Adding a nudge that points at 'no content yet' wastes a hook firing." — `jolly-phonics-apps/.claude/docs/claude-architecture.md:177` and `madrosc-tlumu/.claude/docs/claude-architecture.md:144`
...
**Reversal criterion.** When the primitive sister skill (e.g. `jolly-api`, `tlumu-functions`) accumulates 2+ reference docs, extend the hook with its path nudges.
...
### Example nudge — qbt's security path

```bash
case "$repo_rel" in
  packages/dske/lib/src/ffi/*|\
  packages/dske/lib/src/executor/*|\
...
> Source: `qbt-black-phone/.claude/scripts/bp_quality_check.sh:108-118`
...
> "`bp_skills_drift.sh --all` — Full scan across .claude/ + CLAUDE.md. Emits findings to stderr; exit 1 if any dead link, 0 if clean. `bp_skills_drift.sh` — PostToolUse hook mode — reads JSON from stdin, scans the edited file only." — `qbt-black-phone/.claude/scripts/bp_skills_drift.sh:8-15`
...
> "Out of scope for v1 (by design, to keep signal high): bare path references in prose / code fences, command examples, orphan skills (exist but unreferenced), frontmatter lint. Add incrementally after tuning signal/noise." — `qbt-black-phone/.claude/scripts/bp_skills_drift.sh:20-25`
...
| **Dry-run env var** | `JOLLY_MCP_DRY_RUN=1` lets the user see what would be killed without killing — useful for first-run vetting in a new repo. |
| **Stale threshold tunable** | `JOLLY_MCP_STALE_HOURS=4` default; team-local override via shell rc. |

### Why other repos don't have this hook

qbt and tlumu observed the same Dart analyzer cost but **haven't reported the cumulative leak**. The hook is repo-local because:
...
- **Disjoint scope.** Foundation matches on `*_state.dart`, `*_screen.dart`, `*_view.dart` for hook / Screen-State-View idioms. Project matches on repo-specific paths (`packages/dske/lib/src/ffi/`, `core_messaging/lib/service/crypto*`, `classroom/lib/ui/lesson/activity/*`, etc.). A file that triggers both → both fire, surfacing disjoint nudges.
...
> "**Layers enforce disjoint patterns.** Foundation hooks match on `*_state.dart`, `*_screen.dart`, `*_view.dart`. Project hooks match on `*.g.dart`, `packages/dske/lib/src/ffi/`, `core_messaging/lib/service/crypto*`. If a file triggers both, both fire — that's fine." — `qbt-black-phone/.claude/docs/claude-architecture.md:91-94`
...
A SessionStart hook (when justified — see jolly precedent above) wires under a separate top-level key:
...
> "Only fire inside the jolly-phonics-apps repo." — `jolly-phonics-apps/.claude/scripts/jolly_quality_check.sh:76`
...
✅ Wait until the skill has 2+ references before adding the path nudge. Until then, description matching alone is enough. See `jolly-phonics-apps/.claude/docs/claude-architecture.md:170-179` for the formal rejection.
...
❌ Adding a Dart MCP cleanup hook to a new repo "in case the team has the same problem as jolly".
