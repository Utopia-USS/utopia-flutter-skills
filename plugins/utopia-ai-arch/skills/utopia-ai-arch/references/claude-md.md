...
Two files, two truths, silent drift. The repo-A precedent (a copy where the symlink should be — 16574 bytes of regular file, 2 lines already diverged from CLAUDE.md) is the canonical example.
...
**Fix:** strip CLAUDE.md's `## Architecture decisions` section to one pointer paragraph (repoC / repoB precedents):

> "See `.claude/docs/claude-architecture.md` — skill split rationale, enforcement mode, agent roster choices, rejected alternatives, reversal criteria, toolchain canon." — `production-repo-C/CLAUDE.md:87-89`
