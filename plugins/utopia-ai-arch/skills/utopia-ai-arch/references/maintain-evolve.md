...
**Precedent.** qbt's `bp` / `bp-security` split — engineering surface
...
> from the engineering surface." — `qbt-black-phone/.claude/docs/claude-architecture.md:127`
...
6. **Update `<prefix>_quality_check.sh`** — add a new path-nudge
   ...
   nudges to surface *both* skills on shared paths (qbt's
...
- The reversal-criterion entry from §3 is met ("if `<sister>` never
  ...
  separation pays no rent" — tlumu's pre-recorded reversal).

> "If `tlumu-functions` never accumulates references beyond pointers
> to `.claude/refs/`, the separation pays no rent. Re-consolidate by
> collapsing it back into `tlumu` references and deleting the
> standalone `SKILL.md`." — `madrosc-tlumu/.claude/docs/claude-architecture.md:117`
...
**Precedent.** qbt rejected `bp-repo-map` and `bp-build-verify` for
...
> available at lower context cost." — `qbt-black-phone/.claude/docs/claude-architecture.md:256-258`
...
> wastes a hook firing." — `jolly-phonics-apps/.claude/docs/claude-architecture.md:175-178`

> "Reversal criterion. `tlumu-functions` accumulates 2+ references —
> wire path nudges then." — `madrosc-tlumu/.claude/docs/claude-architecture.md:145`
...
2. **Write the agent file** with `tools: Read, Grep, Glob, Bash`
   ...
   split out a domain skill — qbt's `bp-security` for the
...
5. **Update slash commands** that gate on the domain
   (e.g. qbt's `/bp-team` gates security-sensitive PRs on a
...
6. **Update `claude-architecture.md`** §"Agent roster" with the new
   ...
   ("…considered, deferred until an incident motivates it" is jolly
   and tlumu's standing entry — flip in place when met).
...
adds a layer and splits context. qbt's `/bp-review` was rejected for
exactly this:
...
> context and no wrapper overhead." — `qbt-black-phone/.claude/docs/claude-architecture.md:249-251`
