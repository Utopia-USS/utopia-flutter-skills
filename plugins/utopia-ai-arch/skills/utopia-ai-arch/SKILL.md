...
description: >
  ...
  criteria distilled from production-repo-A, production-repo-B, and production-repo-C.
...
| [evolution-and-drift.md][evolution-and-drift] | CRITICAL | Operations on a live layer (graduate / split / collapse / delete a skill, add/remove path nudges, add a domain auditor mid-project) paired with the 22-symptom drift catalogue from production (repo-A / repoB / repoC). Triggers for re-reading the architecture doc + audit grep one-liners |
...
- **Prefix ≠ repo-folder-name; basename guard uses repo-folder-name.** Hook's `basename "$repo_root"` match MUST use the on-disk folder (e.g. `production-repo-A`), not the prefix (e.g. `bp`). Otherwise the hook silently never fires.
...
Distilled from the production `.claude/` layers of `production-repo-A`, `production-repo-B`, and `production-repo-C`. Built by UtopiaSoftware.
