...
- **Prefix can differ from repo-folder-name; basename guard uses repo-folder-name.** The `<prefix>` is the artifact-name slug (e.g. `bp`); the `<repo-folder-name>` is the on-disk basename (e.g. `production-repo-A`). The hook's `basename "$repo_root"` match MUST use the folder name, not the prefix — otherwise the hook silently never fires.
