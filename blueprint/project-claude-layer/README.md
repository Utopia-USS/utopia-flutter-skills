...
6. **MCP assumption** — for Dart projects, an MCP dart server (e.g.
   `dart-mcp`, `repoB-dart`) is **assumed present** and is the
...
drift — exactly the situation repoB currently has, where `AGENTS.md`
...
what repoB uses for `proto/classroom/classroom_data.proto` ↔
...
5. **Replace placeholders.** Sed `<repo>` → your repo's prefix
   (`bp`, `repoB`, etc.), `<project name>` → human name. Strip the
